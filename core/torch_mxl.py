...
class TorchMXL(nn.Module):
    def __init__(self, dcm_dataset, batch_size, use_cuda=True, use_inference_net=False):
        ...
        if self.dcm_spec.model_type != 'MNL':
            ...
            self.L_omega_offdiag_mu = nn.Parameter(
                torch.ones(int((self.num_mixed_params * (self.num_mixed_params - 1)) / 2)))
            self.L_omega_offdiag_sigma = nn.Parameter(
                torch.zeros(int((self.num_mixed_params * (self.num_mixed_params - 1)) / 2)))
            ...
            if self.dcm_spec.model_type == 'ContextMXL':
                ...
                self.context_fc2 = nn.Linear(self.context_hidden_dim, len(self.dcm_dataset.idx_to_shift))
            ...
            if self.use_inference_net:
                ...
                self.cnn1 = torch.nn.Conv1d(1, self.infnet_hidden_dim, kernel_size=(self.kernel_size),
    ...
    def forward(self, alt_attr, context_attr, obs_choices, alt_avail, alt_ids):
        """
        ...
        if self.dcm_spec.model_type == 'MNL':
            return q_alpha, None, None, None, None, None
        ...
        if self.dcm_spec.model_type == 'ContextMXL':
            ...
            hidden = self.tanh(self.context_fc1(context_attr))
    ...
    def elbo(self, alt_attr, context_attr, obs_choices, alt_avail, obs_mask, alt_ids, indices):
        ...
        q_alpha, q_zeta, q_L_Omega_diag, q_L_Omega_offdiag, q_beta, beta_offsets = self.forward(alt_attr, context_attr,
                                                                                                obs_choices, alt_avail,
                                                                                                alt_ids)

        # samples from posterior approximation
        ...
        if self.dcm_spec.model_type == 'MNL':
            ...
            ## BEGIN This next piece of code fixes the bug of the incorrect order of parameters found by Mirka
            # params_resp = torch.cat([alpha.repeat(self.batch_size,1), beta[indices]], dim=-1)
            next_fixed = 0
            next_mixed = 0
            reordered_pars = []
            ...
            ## END of bug fix
        elif self.dcm_spec.model_type == 'ContextMXL':
            # params_resp = torch.cat([alpha.repeat(self.batch_size,1), beta[indices]], dim=-1)

            # params_resp = params_resp.unsqueeze(0).repeat((self.num_menus,1,1))
            ...
            reordered_pars = []
            reordered_beta_offsets = []

            alpha_resp = alpha.repeat(self.batch_size, 1)

            idx_relation_dict = dict(
                zip(self.dcm_dataset.idx_to_shift, range(len(self.dcm_dataset.idx_to_shift))))

            for par_id in range(self.num_params):
                if par_id in self.dcm_spec.fixed_param_ids:
                    reordered_pars.append(alpha_resp[:, next_fixed].unsqueeze(-1))

                    if par_id in self.dcm_dataset.idx_to_shift:
                        reordered_beta_offsets.append(
                            beta_offsets[:, :, idx_relation_dict[par_id]].unsqueeze(-1))
                    else:
                        reordered_beta_offsets.append(
                            torch.zeros([self.batch_size, self.num_menus, 1]).to(self.device))
            ...
            reordered_pars = torch.cat(reordered_pars, dim=-1)
            reordered_beta_offsets = torch.cat(reordered_beta_offsets, dim=-1)

            reordered_pars = reordered_pars + reordered_beta_offsets.transpose(0, 1)

            beta_resp = torch.cat(
                [reordered_pars[:, :, self.dcm_spec.param_id_map_by_alt[i]] for i in range(self.num_alternatives)],
                dim=-1)
        ...
        loss = td.Categorical(logits=utilities).log_prob(obs_choices.transpose(0, 1))
        loss = torch.where(obs_mask.T, loss, loss.new_zeros(()))  # use mask to filter out missing menus
        loss = loss.sum()
    ...
    def infer(self, num_epochs=10000, true_alpha=None, true_beta=None, true_beta_resp=None):
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, self.num_resp, self.batch_size):
                ...
                elbo = self.elbo(batch_x, batch_context, batch_y, batch_alt_av_mat, batch_mask_cuda, batch_alt_ids,
                                 indices)
                ...
                if not epoch % 100:
                    ...
                    if self.dcm_spec.model_type != 'MNL' and np.all(true_beta != None):
                        beta_rmse = np.sqrt(np.mean((true_beta - self.zeta_mu.detach().cpu().numpy()) ** 2)) if len(
                            true_beta) > 0 else np.inf
                        beta_errors += [beta_rmse]
                        msg += "; Beta RMSE: %.3f" % (beta_rmse,)
                    if self.dcm_spec.model_type != 'MNL' and np.all(true_beta_resp != None):
                        params_resps_rmse = np.sqrt(
                            np.mean((true_beta_resp - self.beta_mu.detach().cpu().numpy()) ** 2)) if len(
                            true_beta_resp) > 0 else np.inf
        ...
        if self.dcm_spec.model_type != 'MNL':
            ...
            if self.use_inference_net:
                q_alpha, q_zeta, q_L_Sigma_diag, q_L_Sigma_offdiag, q_beta, beta_offsets = self.forward(
                    self.train_x.to(self.device), self.context_info.to(self.device), self.train_y.to(self.device),
                    self.alt_av_mat_cuda.to(self.device), self.alt_ids_cuda.to(self.device))
