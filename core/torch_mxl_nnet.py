...
class TorchMXL_NNET(TorchMXL):
    def __init__(self, dcm_dataset, batch_size, use_cuda=True, use_inference_net=False):
        ...
        """
        self.train_nnet = torch.tensor(dcm_dataset.nnet_inputs, dtype=torch.float)#.to(self.device)
    ...
    def initialize_variational_distribution_q(self,):
        ...
        # neural network to be used in the utility functions
        self.nnets_layers = {}
        for alt in range(self.num_alternatives):
            self.nnets_layers[alt] = {}
            for nnet in np.unique(self.dcm_dataset.attr_to_nnet_map):
                input_var_names = self.dcm_dataset.nnet_attr_names[np.where((self.dcm_dataset.nnet_attr_to_alt_map == alt) & (self.dcm_dataset.attr_to_nnet_map == nnet))[0]]
                nnet_input_dim = len(input_var_names)
                nnet_hidden_dim = 5
                layer_seq = []
                #layer_seq.append(nn.BatchNorm1d(nnet_input_dim))
                layer_seq.append(nn.Linear(nnet_input_dim, nnet_hidden_dim).to(self.device))
                layer_seq.append(nn.Dropout(0.5).to(self.device))
                layer_seq.append(nn.Linear(nnet_hidden_dim, 1).to(self.device))
                self.nnets_layers[alt][nnet] = layer_seq
    ...
    def elbo(self, alt_attr, nnet_attr, context_attr, obs_choices, alt_avail, obs_mask, alt_ids, indices):
        """
        ...
        # ----- compute components of the utilities involving the neural network -----
        utilities_nnet = []
        for alt in range(self.num_alternatives):
            #print(self.dcm_dataset.nnet_inputs.shape) # (500, 5, 15)

            utilities_temp = []
            for nnet in np.unique(self.dcm_dataset.attr_to_nnet_map):
                inputs = nnet_attr[:,:,np.where((self.dcm_dataset.nnet_attr_to_alt_map == alt) & (self.dcm_dataset.attr_to_nnet_map == nnet))[0]]
                #print(alt, nnet, inputs.shape, inputs.device)
                hidden = self.nnets_layers[alt][nnet][0](inputs)
                for i in range(1, len(self.nnets_layers[alt][nnet])):
                    hidden = self.nnets_layers[alt][nnet][i](hidden)
                #print(hidden.shape)
                utilities_temp.append(hidden)
            
            utilities_temp = torch.cat(utilities_temp, dim=-1)
            utilities_temp = utilities_temp.sum(dim=-1).unsqueeze(-1)
            #print("utilities_temp:", utilities_temp.shape)
            utilities_nnet.append(utilities_temp)
            
        utilities_nnet = torch.cat(utilities_nnet, dim=-1)
        #print(utilities_nnet.shape)
        utilities += utilities_nnet.transpose(0,1)
    ...
    def infer(self, num_epochs=10000, true_alpha=None, true_beta=None, true_beta_resp=None):
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, self.num_resp, self.batch_size):
                ...
                batch_x, batch_nnet, batch_context, batch_y = self.train_x[indices], self.train_nnet[indices], self.context_info[indices], self.train_y[indices]
                ...
                batch_nnet = batch_nnet.to(self.device)
                ...
                elbo = self.elbo(batch_x, batch_nnet, batch_context, batch_y, batch_alt_av_mat, batch_mask_cuda, batch_alt_ids, indices)
